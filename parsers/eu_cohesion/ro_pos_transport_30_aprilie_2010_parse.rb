...
class EuCohesion::RoPosTransport30Aprilie2010Parse
  ...
  def perform result
    ...
    uri = 'http://localhost:9999/Lista_contractate_POS_Transport_30_aprilie_2010.pdf'
  ...
  def fix_texts texts
    ...
    texts.each_with_index do |text, index|
      if text.left == '546' && (upto = lefts.last(lefts.size - (index +1)).index('546'))
        if upto > 0
          upto.times do |i|
            i = i + 1
...
<text top="766" left="593" width="46" height="12" font="2"><b>TOTAL</b></text>', '')
    ...
    write_csv attribute_keys, attribute_keys, 'eu_cohesion/ro_pos_transport_30_aprilie_2010.csv'
  end
